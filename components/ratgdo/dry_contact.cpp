    ...
    namespace dry_contact {
        ...
        void DryContact::door_action(DoorAction action)
        {
            if (action == DoorAction::OPEN && this->limits_.open_limit_reached) {
                ESP_LOGW(TAG, "The door is already fully open. Ignoring door action: %s", DoorAction_to_string(action));
                return;
            }
            if (action == DoorAction::CLOSE && this->limits_.close_limit_reached) {
                ESP_LOGW(TAG, "The door is already fully closed. Ignoring door action: %s", DoorAction_to_string(action));
                return;
            }
